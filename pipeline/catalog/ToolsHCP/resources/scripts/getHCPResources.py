...
if (Resource is not None):
    ...
    if (getHCP.Session == None):
        ...
        if (isinstance(ResourceMeta, int)):
            print 'ERROR: getHCPResources failed. RESOURCES %s corrupted for subject %s.' % (Resource, Subject)
            sys.exit()
    ...
    for i in xrange(0, len(ResourceMeta.get('Path'))):
        ...
        if (Files == None):
            ...
            # hack for getting just resource level files...
            elif (ResourceMeta.get('Path')[i].find(Resource +'/') > 0) and (ResourcePathSplit[0] == '*'):
                ResourceMetaList = ResourceMeta.get('Path')[i].split('/')
    
                if (Resource in ResourceMetaList):
                    FilePathNameReadable.append(ResourceMeta.get('Readable')[i])
                    FilePathName.append(ResourceMeta.get('RealPath')[i])
                    FileURIName.append(ResourceMeta.get('URI')[i])
                else:
                    print 'ERROR: %s is not in %s.' % (ResourcePath, ResourceMeta.get('Path')[i])
                    # sys.exit()
